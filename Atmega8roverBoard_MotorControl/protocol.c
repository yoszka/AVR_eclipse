...
// ****************** Static variables ******************************************************
// USART part
static VUCHAR aucDataReceived[RECEIVED_BYTE_BUFFER_SIZE];
static VUCHAR ucLastByte = 0;
static VUCHAR  uiReceivedByteCount = 0;
...
BOOL bMotorManualHandler(UCHAR ucInputByte)
{
    ...
    if((ucLastByte == ':') && (ucInputByte == '#') && (uiReceivedByteCount > MINIMUM_DATA_RECEIVED_CNT)){
...
#if DEBUG_OUTPUT
        USART_Transmit_string((UCHAR*)aucDataReceived);
#endif // DEBUG_OUTPUT
        uiReceivedByteCount = 0;

        if(0 == strcmp("*:START:#", (CHAR*)aucDataReceived)){
            USART_Transmit_string((UCHAR*)"*:SERVO_READY:#");
        ...
        if((aucDataReceived[2] == 'S') &&
           (aucDataReceived[3] == 'C'))
        {
            UCHAR ucSysCmd = ucGetThreeDigitValue((UCHAR *)&(aucDataReceived[4]));
        ...
        if(aucDataReceived[2] == 'P'){
            USART_Transmit_string((UCHAR*)"Period = ");
        ...
        if(aucDataReceived[6] == 'H'){
            USART_Transmit_string((UCHAR*)"HIGH = ");
        ...
        if(aucDataReceived[10] == 'R'){
            USART_Transmit_string((UCHAR*)"Repeater = ");
            ...
            // Send Acknowledge
            USART_Transmit_string((UCHAR*)"*:ACK:#");
           ...
           (aucDataReceived[8] == '&'))
        {
            ...
            // VELOCITY
            ucLeftMotorValocity = ucGetThreeDigitValue((UCHAR *)&(aucDataReceived[5]));
            ...
            // VELOCITY
            ucRightMotorVelocity = ucGetThreeDigitValue((UCHAR *)&(aucDataReceived[10]));
